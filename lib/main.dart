...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  double _factorial(int n) {
    if (n < 0)
      return double.nan; // Faktorial tidak terdefinisi untuk bilangan negatif
  ...
  String _formatNumber(String number) {
    NumberFormat numberFormat = NumberFormat("#,##0");
    return numberFormat
        .format(double.tryParse(number.replaceAll(',', '')) ?? 0);
  }
...
class CalculatorButton extends StatelessWidget {
  ...
  Color _getButtonColor(String buttonText) {
    if (buttonText == 'AC') {
      return Color.fromARGB(255, 38, 51, 63);
    } else if (buttonText == '=') {
      return Color.fromARGB(255, 9, 46, 29);
    } else if (buttonText == '()') {
      return Color.fromARGB(255, 46, 78, 69);
    } else if (buttonText == '√' ||
        buttonText == 'π' ||
        buttonText == '^' ||
        buttonText == '!' ||
      ...
      return const Color.fromARGB(255, 27, 21, 20);
    }
  }
}
...
final List<String> _additionalButtonNames = ['sin', 'cos', 'tan', 'x²', 'e'];
